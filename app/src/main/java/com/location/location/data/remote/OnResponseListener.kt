...
abstract class OnResponseListener<in T> : OnListener<T> {
...
interface OnListener<in T> {
    ...
    fun onRequestSuccess(isSuccess: Boolean, response: T, message: String)  // use this method in handle pagination response
