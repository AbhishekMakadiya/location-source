...
class PaginationHelperNestedScrollView<T>(
    private val mContext: Context,
    private val errorLayout: View?,
    val arrayList: ArrayList<T?>?,
    private val recyclerView: RecyclerView,
    private val mNestedScrollView: NestedScrollView,
    ...
    init {
        if (errorLayout != null) {
            ...
            errbutton?.setOnClickListener {
                ...
                paginationCallback.onRetryPageNested(PAGE_INDEX)
            }
        }
    ...
    private fun setPaginationRecycler() {
        ...
        mNestedScrollView.setOnScrollChangeListener { v, scrollX, scrollY, oldScrollX, oldScrollY ->
            val nestedScrollView = checkNotNull(v){ return@setOnScrollChangeListener }

            val lastChild = mNestedScrollView.getChildAt(mNestedScrollView.childCount - 1)

            if (lastChild != null) {
                if ((scrollY >= (lastChild.measuredHeight - nestedScrollView.measuredHeight)) && scrollY > oldScrollY && !isLoadingData) {
                    if (canIncreasePageSize) {
                        isLoadingData = true
                        PAGE_INDEX += 1
                        paginationCallback.onNewPageNested(PAGE_INDEX)
                        if (arrayList!!.size >= PAGE_SIZE) {
                            arrayList.add(null)
                            recyclerView.post {
                                recyclerView.adapter?.notifyItemInserted(arrayList.size - 1)
                        ...
                        if (arrayList!!.size > 0 && arrayList[arrayList.size - 1] != null) {
                            arrayList.add(null)
                            recyclerView.post { recyclerView.adapter?.notifyItemInserted(arrayList.size - 1) }
...
//                    Util.loadGifImage(mContext, progress.findViewById(R.id.imgGif))
    ...
    fun callOnScrollListener(dy: Int) {
    ...
    interface PaginationCallback {
        fun onNewPageNested(pageNumber: Int)
        fun onRetryPageNested(pageNumber: Int)
    }
